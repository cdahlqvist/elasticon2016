...
do
  ...
  curl -XDELETE https://$ES_HOST/shard*/_optimize -u $ES_USER:$ES_PASSWORD

  $RANKIN_PATH/bin/rankin -h $ES_HOST -c $ES_USER:$ES_PASSWORD -r $RUNID_query -i 0 -d 3 -f ./configs/$INDEXNAME_query.json -f ./configs/stats_retrieval.json -a 2
done
